<?php
// Final verification that the updated upload.php works with the new report format
include 'upload.php';

// Simulate processing the new report file
...
// Extract Trades/Deals Table (using the updated function)
...
if (!empty($trades)) {
    ...
    if (!empty($monthlyStats)) {
        echo "<h3>Sample Monthly Data:</h3>";
        echo "<pre>";
        ...
        foreach ($monthlyStats as $key => $stat) {
            ...
            echo "<p>$bulanTahun: " . number_format($stat['net_profit'], 2) . " (" . $stat['jumlah_trade'] . " trades)</p>";
        }
    }
} else {
